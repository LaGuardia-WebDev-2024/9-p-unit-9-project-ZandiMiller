...
draw = function(){
    ...
    if (answer === 1) {
        text("Yesssss!", textX, textY);
    } else if (answer === 2) {
        text("Noooooo!", textX, textY);
    } else if (answer === 3) {
        text("Uh...Maybe?", textX, textY);
    } else if (answer === 4) {
        text("Ask again", textX, textY);
    } else if (answer === 5) {
        text("Prob not🤷‍♂️", textX, textY);
    }
...
mouseClicked = function(){
    ...
    textX = random(160, 240);
    textY = random(180, 260);
    
    unicornX = mouseX;
    unicornY = mouseY;
};

var drawUnicorn = function(x, y){
    translate(x, y);
    scale(0.5);
    //earone
stroke(93,26,110);
